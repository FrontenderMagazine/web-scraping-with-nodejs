...
Сграбить — программно получить данные из интернета. С ростом объёмов данных в нём
грабёж становится всё более распространённым, и для его упрощения возникло множество
мощных сервисов. 

![Иллюстрация][Облако тегов]

К сожалению, большинство из них дороги, на них наложены некие
...
Чтобы установить вышеупомянутые модули в Node.js используется [NPM][3],
...
модули в директорию `node_modules` в той же директории, в которой была выполнена
команда, так что проследите за тем, чтобы выполнить её в папке проекта.
...
других вопросах, которые возникают в процессе грабежа. [Модуль Request][5]
...
главной страничке: «Cheerio это быстрый, гибкий и надёжный порт jQuery,
...
Код, приведённый ниже, это простое приложение, которое получает температуру
...
пытаемся подключить; вы можете узнать как это сделать по ссылкам приведённым выше)
...
Так, и что мы тут делаем? Сначала подключаем модули, чтобы можно было их
использовать в скрипте. Потом сохраняем в переменной URL, который грабим.

Затем используем модуль Request, чтобы скачать страничку, находящуюся по
переданному функции `request` URL. Мы передаём в качестве аргументов URL,
...
в консоль, чтобы увидеть, что пошло не так.

Если всё хорошо, то передаём данные Cheerio. В результате мы можем работать с
данными как на любой веб-страничке, используя стандартный синтаксис jQuery.
Чтобы найти нужные нам данные, надо написать селектор, который получит нужный
...
что большой, зелёный элемент с температурой на нём — тот самый элемент, для
...
Чтобы скачать странички — используем Request, как и в примере выше и парсим его
...
В этом случае, переменная с URL, который мы передаём, поисковый запрос для
термина «data mining».

Как видите, мы сначала делаем запрос, чтобы получить содержимое страницы. Затем
...
так как такая ссылка нам не подходит.
...
окажется массив содержащий все отображённые слова на странице. Мы можем перебрать
...
занимаетесь дата-майнингом, то лучше иметь столько слоёв проверок, сколько
...
комментариях. А ещё можете читать меня в [Twitter][13] и посматривать
мой [блог][15] на предмет новых статей о Node.js, грабеже и JavaScript
в целом.
