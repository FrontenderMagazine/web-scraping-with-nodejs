...
обладают какими-либо недостатками. Вместо того, чтобы пользоваться
...
теме, [прочтите-ка это][1], прежде чем продолжить. Кроме того,
...
слышали о Bower — это то же самое, за тем исключением того, что 
...
устанавливает модули в директорию `node_modules` в той же директории, 
...
Так, и что мы тут делаем? Сначала подключаем модули, чтобы позже ими
воспользоваться, затем сохраняем в переменной URL, содержимое которого 
собираемся «сграбить».
...
который хотим скачать и колбек, который обработает результаты запроса.
Когда мы получим данные, будет вызван колбек, в который будут переданы в качестве
аргументов три переменных: `error`, `responce` и `body`. Если Request не сможет
скачать страничку и получить данные, он передаст в функцию объект в переменной
`error` и `null` в качестве аргумента `body`. Прежде чем начинать работать
...
который получит нужный элемент (или набор элементов). Если вы откроете
в браузере URL из примера и исследуете страницу с помощью
инструментов разработчика, то увидите, что большой зелёный блок с 
...
каждой страницы, ссылки на которые будут на первой странице результатов Google.

В приложении есть три главных части:
...
Давайте посмотрим на код, который позволит реализовать каждый из этих пунктов —
...
результаты, чтобы получить нужные URL нужных нам страниц.
...
Если так — это внутренняя ссылка на ресурс Google, и нам её содержимое скачивать не
нужно.

### Получаем слова с каждой страницы

Теперь, когда у нас есть URL страниц, надо получить слова с каждой из них. Этот
шаг представляет собой ровно то же, что мы уже делали ранньше — только теперь URL
...
её DOM. В данном случае, мы получаем только текст со страницы.
...
JSON. Нам нужно сделать вот что:
...
Когда мы это сделаем, можно будет просто разбить текст по пробелам, и мы получим
массив, содержащий все слова со страницы. Можно перебрать
его в цикле и добавить слова в словарь.
...
возможно. При всём многообразии данных, которое можно получить таким образом,
